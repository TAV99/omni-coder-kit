...
const { UNIVERSAL_SKILLS, UI_SKILLS, TEST_SKILLS, FE_SKILLS, SHIP_SKILLS, QUALITY_SKILLS } = require('../skills');
...
/**
 * Gom toàn bộ `source` từ mọi registry hardcode, dedupe, kèm danh sách skill dùng nguồn đó.
...
function scanInstalledSkills(projectDir, manifest) {
    ...
    const results = entries.map((name) => {
        ...
        const validated = validateSkillContent(name, content);

        const ext = manifest.skills?.external?.find(s => s.name === name);
        if (ext && (ext.sourceType === 'bundled' || ext.sourceType === 'cached')) {
            const label = ext.sourceType === 'bundled' ? 'bundled (offline)' : 'cached (offline)';
            validated.warnings.push(`Cài từ ${label}. Chạy 'omni skills -y' khi có mạng để cập nhật.`);
        }
