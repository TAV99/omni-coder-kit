...
const IDE_DISPLAY_NAME = {
    claudecode: 'Claude Code', gemini: 'Gemini CLI', codex: 'Codex CLI', dual: 'Claude Code + Codex',
...
async function handleInit(options) {
    ...
    const response = await prompts([
        ...
        },
        {
            type: 'select',
            name: 'mode',
            message: q(2, 3, 'Chế độ chạy SDLC?'),
            choices: [
    ...
    let dualPair = 'claude-codex';
    if (response.ide === 'dual') {
        const dp = await prompts({
            type: 'select',
            name: 'dualPair',
            message: 'Chọn cặp phối hợp Dual Mode:',
            choices: [
                { title: 'Claude Code + Codex (mặc định)', value: 'claude-codex' },
                { title: 'Codex + Gemini qua Antigravity (agy)', value: 'codex-agy' },
            ],
            initial: 0,
        }, { onCancel });
        if (cancelled) return;
        dualPair = dp.dualPair || 'claude-codex';
    }
