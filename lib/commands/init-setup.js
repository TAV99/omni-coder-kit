...
const {
    ...
    buildAntigravityMcpConfig, buildAntigravityExtension, buildAntigravitySkills,
...
async function maybeInstallAntigravityGlobal({ extension, mcpConfig, hooks, skills, onCancel }) {
...
async function setupAntigravityAdvanced(manifest, ide, initFiles, parsedRules, rulesContent, strictness, fileName) {
    ...
    if (antigravityAdvanced) {
        ...
        // Native agy skills: .agent/skills/<name>/SKILL.md → /<name>
        const skills = buildAntigravitySkills(ide);
        if (skills) {
            for (const s of skills) {
                const skillDir = path.join(projectDir, '.agent', 'skills', s.name);
                fs.mkdirSync(skillDir, { recursive: true });
                fs.writeFileSync(path.join(skillDir, 'SKILL.md'), s.content);
            }
            console.log(chalk.green(`   ✅ .agent/skills/ (${skills.length} native skills — /om-*)`));
        }
        ...
        await maybeInstallAntigravityGlobal({ extension, mcpConfig, hooks, skills, onCancel });
        if (cancelled) return;
