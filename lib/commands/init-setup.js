...
async function setupCodexAdvanced(manifest, ide) {
    ...
    const codexConfig = buildCodexConfig(ide, codexAdvanced, manifest);
    const codexHooks = buildCodexHooks(ide, codexAdvanced, manifest);
...
async function setupAntigravityAdvanced(manifest, ide, initFiles, parsedRules, rulesContent, mode, fileName, options = {}) {
    const projectDir = process.cwd();
    const agyProjectId = registerAntigravityProject(projectDir);
    if (agyProjectId) {
        manifest.agyProjectId = agyProjectId;
        saveManifest(manifest);
    }
    let cancelled = false;
    const onCancel = () => { cancelled = true; };
    ...
    const { antigravityAdvanced } = await prompts({
        ...
        message: options.promptMessage || '🔧 Cài đặt Antigravity nâng cao? (Modular rules in .agents/rules/)',
        initial: false
    }, { onCancel });
    if (cancelled) return;
    ...
    if (antigravityAdvanced) {
        ...
        if (options.preserveConfigFile) {
            console.log(chalk.gray(`   ↳ Giữ AGENTS.md của Dual mode; agy dùng .agents/ configuration.`));
        } else {
            const personalRulesBlock = rulesContent
                ? `\n<!-- omni:rules -->\n## PERSONAL RULES\n${formatInject(parsedRules)}\n<!-- /omni:rules -->\n`
                : '';
