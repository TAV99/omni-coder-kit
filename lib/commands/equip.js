...
function buildSkillInstallArgs(skill, agentFlags) {
    const args = ['-y', 'skills', 'add', skill.source];
    if (agentFlags) args.push(...agentFlags.split(' '));
    args.push('--skill', skill.name, '-y');
    return args;
}
    ...
    if (isWin) {
        ...
        for (const skill of failedSkills) {
            const installCmd = agentFlags
                ? `npx -y skills add ${skill.source} ${agentFlags} --skill "${skill.name}" -y`
                : `npx -y skills add ${skill.source} --skill "${skill.name}" -y`;
...
async function handleAutoEquip(options) {
    ...
    if (toInstall.length === 0) {
        ...
        for (const skill of toInstall) {
            ...
            try {
                const skillArgs = buildSkillInstallArgs(skill, agentFlags);
