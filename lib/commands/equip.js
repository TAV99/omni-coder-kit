...
const {
    ...
    classifySource, getSkillDir, fetchContentSync, githubRawURL,
} = require('../helpers');
const { detectExistingProject, scanProject } = require('../scanner');
const { UNIVERSAL_SKILLS, getTestSkillsForStack, getFESkillsForStack, getShipSkillsForStack, getQualitySkillsForStack, buildSearchSuggestion } = require('../skills');
const {
...
function buildNoSkillMsg(techStack) {
    ...
    return {
        ...
        line2: `Gợi ý: dùng >om:equip hoặc tìm: npx skills search "${keyword}"`,
    };
}
