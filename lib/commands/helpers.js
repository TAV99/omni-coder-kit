...
function ensureGitignore(ide) {
    ...
    if (ide === 'windsurf') patterns.push('.windsurf/');
    if (ide === 'gemini') patterns.push('.gemini/');
...
function writeFileSafe(filePath, content) {
    try {
        ...
        console.error(chalk.red(`   Chi tiết: ${err.message}\n`));
        return false;
    }
}
...
function updateOnboardStatus(manifest, generated) {
    manifest.onboard = {
        status: 'completed',
