...
async function handleRun(options = {}) {
    ...
    const onEvent = (e) => {
        ...
        else if (e.type === 'intake') console.log(chalk.cyan(`   📥 intake: ${e.count || 0} requirements${e.skipped ? ` (${e.skipped})` : ''}`));
        else if (e.type === 'acceptance') console.log(chalk[e.allMet ? 'green' : 'yellow'](`   ⚖  acceptance: ${e.met || 0}/${e.total || 0} met${e.failed && e.failed.length ? ` — unmet ${e.failed.join(',')}` : ''}`));
...
module.exports = { handleRun, handleTrace, handleGate, handleStats, handleAccept };
