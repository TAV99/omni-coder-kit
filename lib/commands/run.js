...
function renderRunEvent(e) {
    ...
    switch (e.type) {
        ...
        case 'debate': {
            ...
            out.push(chalk.magenta(`   ⚔️  Tranh luận Vòng ${round}: ${consensus}`));
            for (const p of e.participants || []) {
                const color = p.verdict === 'pass' ? chalk.green : (p.verdict === 'fail' ? chalk.red : chalk.yellow);
                const roleMark = p.verdict === 'pass' ? '✓' : (p.verdict === 'fail' ? '✗' : '?');
                out.push(color(`     ${roleMark} [${p.id}] Voted ${p.verdict.toUpperCase()}${p.summary ? `: "${p.summary}"` : ''}`));
            }
            break;
        }
        case 'debate-warning':
