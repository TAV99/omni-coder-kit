...
function handleUpdate() {
    ...
    let latest;
    try {
        latest = execFileSync(isWin ? 'npm.cmd' : 'npm', ['view', 'omni-coder-kit', 'version'], { encoding: 'utf-8', shell: isWin }).trim();
    } catch {
    ...
    try {
        execFileSync(isWin ? 'npm.cmd' : 'npm', ['install', '-g', 'omni-coder-kit@latest'], { stdio: 'inherit', timeout: 60000, shell: isWin });
