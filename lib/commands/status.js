...
function handleStatus() {
    ...
    console.log(chalk.cyan.bold('   🌐 Skills đã cài (omni skills):'));
...
function handleCommands() {
    ...
    const commands = [
        { cmd: '>om-go',         slash: '/om-go',         role: 'All-in-one',  desc: 'One-shot pipeline — brainstorm/intake → cook → check → ACCEPTANCE → doc (vibe coder)' },
        { cmd: '>om-spec',     slash: '/om-spec',     role: 'Acceptance', desc: 'Spec/Q&A khách hàng → .omni/sdlc/requirements.md (checklist nguyên tử)' },
        { cmd: '>om-pass',     slash: '/om-pass',     role: 'Acceptance', desc: 'Chấm sản phẩm vs từng requirement (test/agent+debate) → conformance.md; lặp tới 100%' },
        { cmd: '>om-scan',    slash: '/om-scan',    role: 'Architect',  desc: 'Onboard dự án cũ — scan code, interview, sinh rules + skills + spec' },
        { cmd: '>om-think', slash: '/om-think', role: 'Architect',  desc: 'Phỏng vấn yêu cầu → đề xuất Tech Stack → xuất .omni/sdlc/design-spec.md' },
        { cmd: '>om-skill',      slash: '/om-skill',      role: 'Skill Mgr',  desc: 'Cài universal skills + tìm & đề xuất skills từ skills.sh theo design-spec' },
        ...
        { cmd: '>om-memo',      slash: '/om-memo',        role: 'Learner',     desc: 'Đúc kết bài học từ bug fix → ghi vào knowledge-base.md (auto sau >om-fix)' },
