...
function createDualOrchestrator(options = {}) {
    ...
    async function executeAgyWithRetry({
        ...
        if (!cachedAgyVersion) {
            ...
            if (persistedVersion && persistedModel === 'gemini-3.7-flash-high') {
                ...
                if (!authVersion || authModel !== 'gemini-3.7-flash-high') {
    ...
    async function probeStandaloneCapability(repoRoot) {
        ...
        try {
            ...
            throw new DualOrchestratorError(
                ...
                'Failed to query models or model gemini-3.7-flash-high is not available',
                { cause: error.message },
            );
        }
        if (
            ...
            !parseAgyModelsOutput((modelsResult.stdout || '').trim())
        ) {
            throw new DualOrchestratorError(
                'DUAL_PREFLIGHT_MODEL_UNAVAILABLE',
                'Failed to query models or model gemini-3.7-flash-high is not available',
    ...
    async function runPhase(phase, taskId) {
        ...
        if (phase === 'preflight') {
            ...
            if (authoritativeCapabilityEvidence) {
                ...
                if (
                    !authVersion ||
                    authModel !== 'gemini-3.7-flash-high'
                ) {
                    throw new DualOrchestratorError(
                        'DUAL_PREFLIGHT_AUTHORITY_INVALID',
                        'Authoritative capability evidence is invalid, missing required model gemini-3.7-flash-high, or missing actual version'
                    );
                }
        ...
        if (phase === 'route') {
            ...
            const routeData = {
                ...
                owner,
                model: isCodex ? null : 'gemini-3.7-flash-high',
                effort: isCodex ? null : 'high',
                token_budget: null,
