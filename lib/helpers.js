...
function detectDNA(projectDir) {
    ...
    try {
        ...
        if (fs.existsSync(reqPath)) pythonDeps += fs.readFileSync(reqPath, 'utf-8');
        const pyprojPath = path.join(projectDir, 'pyproject.toml');
        if (fs.existsSync(pyprojPath)) pythonDeps += fs.readFileSync(pyprojPath, 'utf-8');
    } catch (_) {}
    const hasPyDep = (name) => new RegExp(`\\b${name}\\b`, 'i').test(pythonDeps);
