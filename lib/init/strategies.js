...
function buildGeminiBootstrapContent(strictnessBlock, personalRulesBlock) {
    let content = `> Generated by Omni-Coder Kit\n\n`;
    content += strictnessBlock + '\n';
...
const ANTIGRAVITY_SKILL_DESC = {
    'om:brainstorm': 'Interview the user and advise on architecture before any code. Use at the start of a new feature/project.',
    ...
    'om:learn': 'Persist findings into the knowledge base. Use to capture reusable patterns/solutions.',
    'om:equip': 'Find and install relevant skills. Use to extend the agent for the current stack.',
    'om:onboard': 'Onboard onto an existing codebase. Use when starting on an unfamiliar repo.',
};

// Convert the om:* command bodies into VERIFIED agy native skills:
//   .agent/skills/<name>/SKILL.md (frontmatter name+description) → /<name>.
...
function buildAntigravitySkills(ide) {
    ...
    for (const f of fs.readdirSync(commandsDir).filter(f => f.endsWith('.md'))) {
        ...
        const skillName = cmd.replace(':', '-');      // e.g. "om-cook" → /om-cook
        const body = fs.readFileSync(path.join(commandsDir, f), 'utf-8').trim();
        const desc = ANTIGRAVITY_SKILL_DESC[cmd] || `Omni-Coder Kit ${cmd} workflow.`;
        const content = `---\nname: ${skillName}\ndescription: ${desc}\n---\n\n${body}\n`;
...
function buildWindsurfBootstrapRules(strictnessBlock, personalRulesBlock) {
...
function buildAntigravityBootstrapRules(strictnessBlock, personalRulesBlock) {
    let bootstrap = `> Generated by Omni-Coder Kit\n\n`;
    bootstrap += strictnessBlock + '\n';
    bootstrap += `## RULES SYSTEM\n`;
