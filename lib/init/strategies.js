...
function buildCodexGeminiFiles() {
    ...
    const promptsDir = path.join(codexGeminiDir, 'prompts');
    if (fs.existsSync(promptsDir)) {
        for (const file of fs.readdirSync(promptsDir)) {
            result.push({
                path: path.join('.omni', 'codex-gemini', 'prompts', file),
                content: fs.readFileSync(path.join(promptsDir, file), 'utf-8'),
                overwritePrompt: false,
            });
        }
    }

    const schemasDir = path.join(codexGeminiDir, 'schemas');
    if (fs.existsSync(schemasDir)) {
        for (const file of fs.readdirSync(schemasDir)) {
            result.push({
                path: path.join('.omni', 'codex-gemini', 'schemas', file),
                content: fs.readFileSync(path.join(schemasDir, file), 'utf-8'),
                overwritePrompt: false,
            });
        }
    }
        ...
        if (ide === 'dual' && dualPair === 'codex-agy' && mode === 'auto') {
            ...
            if (thinkSkill) {
                thinkSkill.content += [
                    ...
                    '## Dual Auto Router',
                    '',
                    'After the user-approved design is complete, continue to planning. For each planned task, Codex must write a bounded `spec.json`, run `$omni-codex-gemini` in order: `new → preflight → scout → route`; only when `route.json` owner is `gemini`, continue `implement → review` before Codex final QC.',
                    '- Gemini worker model: `gemini-3.7-flash-high` with high effort.',
                    '- Keep architecture, security, migrations, cross-module, ambiguous, and final-QC work in Codex.',
                    '- Gemini never commits, pushes, deploys, or bypasses permissions; Codex verifies evidence, review, diff, and final tests.',
                    '',
                ].join('\n');
            }
        }
    ...
    if (ide === 'dual') {
        ...
        if (dualPair === 'codex-agy') {
            ...
            dirs.push(
                ...
                path.join('.omni', 'codex-gemini', 'prompts'),
                path.join('.omni', 'codex-gemini', 'schemas')
            );
        }
    }
