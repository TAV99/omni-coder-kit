...
const ANTIGRAVITY_SKILL_DESC = {
    ...
    'om:intake': 'Convert a customer spec / Q&A into .omni/sdlc/requirements.md (atomic, verifiable checklist). Use to lock the contract before coding.',
    'om:accept': 'Acceptance loop — grade product vs each requirement (test/agent+debate), write conformance.md, loop until 100% met. Use after >om:check passes when requirements.md exists.',
...
function buildModeBlock(mode) {
    const common = '## RUN MODE\n'
        ...
        + '- Khi người dùng gọi `>om:brainstorm`, PHẢI kích hoạt đầy đủ quy trình phỏng vấn (DNA detection + adaptive interview) như thường.\n';
    if (mode === 'auto') {
        return common
            + '- **Chế độ AUTO:** Ngay sau khi phần hỏi–đáp của `>om:brainstorm` kết thúc (đã đủ thông tin, đã ghi design-spec), '
            + 'TỰ ĐỘNG chạy tiếp chuỗi mà KHÔNG hỏi giữa các bước: '
            + '`>om:equip` (tự cài, không cần xác nhận) → `>om:plan` → `>om:cook` (đã gồm auto check→fix). '
    ...
    return common
        ...
        + '(brainstorm → equip → plan → cook → check → doc). KHÔNG tự nối chuỗi.\n';
}
...
function buildSupportingSection(sdlcDesc) {
    return [
        ...
        '',
        '**Recommended:** For larger tasks, run `>om:brainstorm` and `>om:plan` first. Brainstorm/plan are OPTIONAL — direct coding is allowed. Guard kept: do NOT `>om:ship` before `>om:check` passes.',
