...
function create({ ide = 'claudecode', timeoutMs = 10 * 60 * 1000, runCommand = realRun, modelByStep = null, yolo = false } = {}) {
    const modelFor = (step) => {
    ...
    return {
        name: 'host-cli',
        buildCommand: (step, ctx = {}) => buildCommand(ide, promptFor(step, ctx), { model: modelFor(step), yolo }),
        async runStep(step, ctx = {}) {
            const built = buildCommand(ide, promptFor(step, ctx), { model: modelFor(step), yolo });
