...
function buildCommand(ide, prompt, opts = {}) {
    switch (ide) {
        ...
        case 'antigravity': {
            ...
            // Wrap in script PTY on Linux/macOS if we are headlessly running.
            if (!opts.skipPTY) {
                const isLinux = process.platform === 'linux';
                const isMac = process.platform === 'darwin';
                if (isLinux) {
                    const escapedCmd = argv.map(arg => {
                        // Escape \, ", $, ` for safe shell double-quoting
                        const escaped = arg
                            .replace(/\\/g, '\\\\')
                            .replace(/"/g, '\\"')
                            .replace(/\$/g, '\\$')
                    ...
                    return { argv: ['script', '-q', '-e', '-c', escapedCmd, '/dev/null'] };
                } else if (isMac) {
                    return { argv: ['script', '-q', '/dev/null', ...argv] };
                }
            }
