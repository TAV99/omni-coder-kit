...
function promptFor(step, ctx) {
    ...
    // Anchor the working directory in-prompt (SPEC-FIX-ANTIGRAVITY-WORKSPACE §4).
    // Harmless for claude/codex/gemini; rescues agy which otherwise drifts into
    // ~/.gemini/antigravity-cli/scratch even with --add-dir set.
    const dirLine = ctx.projectDir
        ? `Your working directory is \`${ctx.projectDir}\`. Read, write, and run ALL files there — do NOT use any scratch directory.\n`
        : '';
    return `${dirLine}Read \`${wf}\` and execute the >om:${step} workflow strictly. `
