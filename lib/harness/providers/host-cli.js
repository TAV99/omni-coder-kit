...
// Default timeouts per step (in minutes) for the agent loop (PLAN/COOK are heavy).
// When stepTimeoutMs is not provided from CLI, these are resolved per-step.
const DEFAULT_TIMEOUT_BY_STEP = {
    intake: 15, brainstorm: 15, equip: 10,
    plan: 20, cook: 25, fix: 12, check: 12, security: 12,
    doc: 12, ship: 10,
};
const FALLBACK_TIMEOUT_MIN = 10;
...
function buildCommand(ide, prompt, opts = {}) {
    switch (ide) {
        ...
        case 'open-claude':
        case 'openclaude': {
            const bin = { claudecode: 'claude', dual: 'claude', opencode: 'opencode', 'open-claude': 'open-claude', openclaude: 'openclaude' }[ide] || 'claude';
...
function create({
    ...
    const resolveTimeoutMs = (step) =>
        (typeof timeoutMs === 'number' && timeoutMs > 0)
            ? timeoutMs
            : (DEFAULT_TIMEOUT_BY_STEP[step] || FALLBACK_TIMEOUT_MIN) * 60 * 1000;

    return {
        name: 'host-cli',
        ide,
        modelFor,
        buildCommand: (step, ctx = {}) => {
            const tMs = resolveTimeoutMs(step);
            return buildCommand(ide, promptFor(step, ctx), {
                model: modelFor(step), yolo,
                projectDir: ctx.projectDir,
                printTimeoutSec: Math.max(30, Math.round(tMs / 1000) - 30),
                skipPTY,
            });
        },
        async runStep(step, ctx = {}) {
            ...
            const tMs = resolveTimeoutMs(step);
            const stepPrintTimeoutSec = Math.max(30, Math.round(tMs / 1000) - 30);
            const built = buildCommand(ide, promptFor(step, ctx), {
                model, yolo,
                projectDir: ctx.projectDir,
                printTimeoutSec: stepPrintTimeoutSec,
                skipPTY,
            });
            ...
            try {
                res = useAsync
                    ? await runCommandAsync(built.argv, { cwd: ctx.projectDir, timeoutMs: tMs, onStdout, signal: ctx.signal })
                    : runCommand(built.argv, { cwd: ctx.projectDir, timeoutMs: tMs });
                ...
                if (wasPTY && (res.exitCode === 127 || (res.exitCode !== 0 && (Date.now() - started) < 1000 && /script:.*not found|command not found/i.test(res.stderr || '')))) {
                    ...
                    res = useAsync
                        ? await runCommandAsync(unwrapped.argv, { cwd: ctx.projectDir, timeoutMs: tMs, onStdout, signal: ctx.signal })
                        : runCommand(unwrapped.argv, { cwd: ctx.projectDir, timeoutMs: tMs });
                }
            } catch (e) {
            ...
            if (res.timedOut) {
                return { ok: false, exitCode: 124, summary: `>om-${step} timed out sau ${Math.round(tMs / 1000)}s`, durationMs, timedOut: true, timeoutMs: tMs, stderrTail, model };
            }
