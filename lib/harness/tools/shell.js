...
function runCommandAsync(cmdOrArgv, {
    ...
    return new Promise((resolve) => {
        const child = isArgv
            ? spawnFn(cmdOrArgv[0], cmdOrArgv.slice(1), { cwd, env: execEnv, stdio: ['pipe', 'pipe', 'pipe'], shell: isWin })
            : spawnFn(cmdOrArgv, { cwd, shell: true, env: execEnv, stdio: ['pipe', 'pipe', 'pipe'] });

        // Close stdin for agy/gemini to prevent them from blocking on stdin read
        const exe = isArgv ? cmdOrArgv[0] : String(cmdOrArgv).split(/\s+/)[0];
        const exeLower = (exe || '').replace(/^["']|["']$/g, '').toLowerCase();
        if (exeLower === 'agy' || exeLower === 'gemini') {
