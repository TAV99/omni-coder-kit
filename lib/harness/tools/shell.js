...
function runCommandAsync(cmdOrArgv, {
    ...
    return new Promise((resolve) => {
        const child = isArgv
            ? spawnFn(cmdOrArgv[0], cmdOrArgv.slice(1), { cwd, env: env || process.env, stdio: ['ignore', 'pipe', 'pipe'] })
            : spawnFn(cmdOrArgv, { cwd, shell: true, env: env || process.env, stdio: ['ignore', 'pipe', 'pipe'] });
        let stdout = '';
        let stderr = '';
        let timedOut = false;
        let settled = false;
