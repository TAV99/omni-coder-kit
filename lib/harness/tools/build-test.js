...
function detectCommands(projectDir) {
    ...
    // Generic Python projects (including Pygame, PyQt6/PySide6, generic Py apps)
    let hasPythonFiles = false;
    try {
        hasPythonFiles = fs.readdirSync(dir).some(f => f.endsWith('.py')) || 
                         fs.existsSync(path.join(dir, 'requirements.txt')) ||
                         fs.existsSync(path.join(dir, 'pyproject.toml')) ||
                         fs.existsSync(path.join(dir, 'setup.py')) ||
                         fs.existsSync(path.join(dir, 'poetry.lock'));
    } catch (_) {}

    if (hasPythonFiles) {
        let mainFile = null;
        for (const candidate of ['main.py', 'app.py', 'run.py']) {
            if (fs.existsSync(path.join(dir, candidate))) {
                mainFile = candidate;
                break;
            }
        }
        out.dev = mainFile ? `python ${mainFile}` : null;
        
        const hasPytest = fs.existsSync(path.join(dir, 'pytest.ini')) || 
                          fs.existsSync(path.join(dir, 'tests')) || 
                          fs.existsSync(path.join(dir, 'test'));
        out.test = hasPytest ? 'pytest' : 'python -m unittest discover';
        
        let linter = 'ruff check .';
        try {
            require('child_process').execSync('ruff --version', { stdio: 'ignore' });
        } catch {
            linter = 'python -m compileall .';
        }
        out.lint = linter;
