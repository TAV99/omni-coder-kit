...
async function runHarness(projectDir, opts = {}) {
    ...
    const step = async (name) => {
        ...
        const r = await prov.runStep(name, { projectDir: dir, state, workflowPath, sharedBrief: brief });
    ...
    while (!TERMINAL.includes(state.state)) {
        ...
        switch (state.state) {
            ...
            case 'CHECK': {
                ...
                const t = parseTodo(dir);
                if (!gate.passed) go('FIX', `gate fail ${gate.failures.join(',')}`);
                else if (t.remaining > 0) go('COOK', 'gate pass, more tasks');
