        ...
        switch (state.state) {
            ...
            case 'ACCEPTANCE': {
                ...
                if (participants.length === 1) {
                    // Clone the specified participant into 2 virtual participants for self-debate
                    const p = participants[0];
                    participants = [
                        { id: `${p.id}_1`, host: p.host, provider: p.provider },
                        { id: `${p.id}_2`, host: p.host, provider: p.provider },
                    ];
                } else if (participants.length === 0) {
                    // Fallback to active runner provider to create 2 virtual debate participants (self-debate/reflection)
                    participants = [
                        { id: `${provider}:${ide || 'default'}_1`, host: ide || provider, provider: prov },
                        { id: `${provider}:${ide || 'default'}_2`, host: ide || provider, provider: prov },
                    ];
                }
