...
async function runHarness(projectDir, opts = {}) {
    ...
    const onStdoutRaw = stream
        ...
        : null;
    const prov = getProvider(provider, { ide, yolo, async: true, stream, onStdout: onStdoutRaw });
    ...
    const escalateOnDebate = async (phase, question) => {
        ...
        const participants = debateSpecs.map((s) => getProviderFromSpec(s, { ide, yolo }));
        ...
        switch (state.state) {
            ...
            case 'ACCEPTANCE': {
                ...
                const participants = Array.isArray(acceptSpecs) && acceptSpecs.length
                    ? acceptSpecs.map((s) => getProviderFromSpec(s, { ide, yolo }))
                    : (Array.isArray(debateSpecs) && debateSpecs.length
                        ? debateSpecs.map((s) => getProviderFromSpec(s, { ide, yolo }))
                        : []);
