...
async function runHarness(projectDir, opts = {}) {
    ...
    let state = (freshStart ? null : loadState(dir)) || createState({ provider, from });
    ...
    if (specFile || specText) {
        ...
        if (txt.trim()) {
            const intakeRes = await buildRequirements({ projectDir: dir, specText: txt, provider: prov, ide, force: !!reintake });
    ...
    const step = async (name) => {
        ...
        state.consecutiveTimeouts = r.timedOut ? ((state.consecutiveTimeouts || 0) + 1) : 0;
        return r;
    };
    const go = (to, reason) => {
    ...
    const blockedOnTimeouts = () => {
        if ((state.consecutiveTimeouts || 0) < budget.maxConsecutiveTimeouts) return false;
        const mins = Math.round((stepTimeoutMs || 600000) / 60000);
        const marked = markBlockedTask(dir);
        const reason = `Agent timeout ${state.consecutiveTimeouts} lần liên tiếp (mỗi lần ~${mins}m) — task quá lớn hoặc agent treo. `
            + `Chia nhỏ task / tăng --step-timeout / kiểm tra agent (vd agy cần --add-dir đúng) / đổi provider.`;
    ...
    while (!TERMINAL.includes(state.state)) {
        ...
        switch (state.state) {
            ...
            case 'COOK': {
                ...
                await step('cook');
                if (blockedOnTimeouts()) return state;
            ...
            case 'CHECK': {
                const dirtyFiles = gitStatusSet(dir);
                let only = null;
                if (dirtyFiles && dirtyFiles.size > 0) {
                    const logicExtensions = ['.js', '.jsx', '.ts', '.tsx', '.py', '.go', '.rs', '.rb', '.php', '.java', '.cpp', '.c', '.h', '.cs', '.kt', '.swift', '.sh', '.bat', '.pl'];
                    const hasLogicChanges = [...dirtyFiles].some((f) => {
                        const ext = path.extname(f).toLowerCase();
                        return logicExtensions.includes(ext);
                    });
                    if (!hasLogicChanges) {
                        only = ['P1', 'P2', 'P4', 'P5'];
                        emit({ type: 'gate-bypass', reason: 'Only non-logic files changed. Bypassing security scan and test gates.' });
                    }
                }
                const gate = runPipeline(dir, { only });
                ...
                if (!gate.passed || lensFail) {
                    ...
                    state.cameFromFix = false;
                    // A fix that preceded this passing CHECK is a confirmed lesson (auto >om-memo).
            ...
            case 'ACCEPTANCE': {
                ...
                let participants = Array.isArray(acceptSpecs) && acceptSpecs.length
                ...
                if (participants.length === 0) {
                    // Fallback to active runner provider to create 1 participant (single-model audit, no debate)
                    participants = [
                        { id: `${provider}:${ide || 'default'}`, host: ide || provider, provider: prov },
                    ];
                }
            ...
            case 'SHIP':
                ...
                break;
            case 'BLOCKED':
                // Resume recovery: user has (presumably) fixed the issue externally.
                // Transition back to CHECK to re-validate. Reset fix counters so the
                // agent gets a fresh set of attempts if a new issue arises.
                state.fixAttempts = 0;
                state.noProgress = 0;
                state.cameFromFix = false;
                state.lastFailures = null;
                ...
                emit({ type: 'recovery', from: 'BLOCKED', to: 'CHECK', reason: 'resume from BLOCKED — re-running quality gates' });
                go('CHECK', 'resume recovery: re-check after user fix');
                break;
            default:
                // Unknown state — escalate / stop.
                state.status = 'error';
                saveState(dir, state);
                return state;
        }
