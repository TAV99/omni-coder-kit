...
// Run one agent-judged requirement via cross-model debate. participants ≥2 and
// (recommended) different hosts. consensus 'agree' && verdict 'pass' → met.
async function runAgentJudgement({
    projectDir, requirement, participants, rounds, runDebate, onEvent,
}) {
    if (!Array.isArray(participants) || participants.length < 2) {
        return { met: false, evidence: 'debate needs ≥2 participants; none configured → cannot judge → NOT met (escalate)' };
    }
    const artifactPaths = [
