...
// Advisory patterns: flagged as warnings in output but do NOT fail the gate.
// innerHTML is common and legitimate in Vanilla JS / no-framework projects.
const ADVISORY_PATTERNS = [
    { re: /\.innerHTML\s*=/, what: 'innerHTML assignment (XSS risk — review manually)' },
];
...
function scanFiles(projectDir) {
    ...
    for (const file of files) {
        ...
        for (const { re, what } of ADVISORY_PATTERNS) {
            if (re.test(content)) warnings.push(`${rel}: advisory — ${what}`);
        }
    }
...
function runSecurity(projectDir, { runCommand = realRun } = {}) {
    const { findings, warnings } = scanFiles(projectDir);
    ...
    if (hasLock && fs.existsSync(path.join(projectDir, 'package.json'))) {
        ...
        if (r && r.exitCode !== 0 && !r.timedOut) {
            warnings.push('npm audit: lỗ hổng mức high+ (chạy `npm audit` để xem chi tiết)');
        }
    }

    // Combine output: hard findings (block) + advisory warnings (info only)
    const outputParts = [];
    if (findings.length) outputParts.push(...findings);
    if (warnings.length) outputParts.push('--- advisory (non-blocking) ---', ...warnings);

    return {
        ran: true,
        passed: findings.length === 0,
        output: outputParts.length ? outputParts.join('\n') : 'no security findings',
    };
}
