...
function generateMapSkeleton(scan, projectName, opts = {}) {
    const date = new Date().toISOString().split('T')[0];
    const lines = [];

    const isClaude = ['claudecode', 'dual'].includes(opts.ide);
    const mapCmd = isClaude ? '/om:map' : '>om:map';
    ...
    lines.push(`> Điền chi tiết: \`${mapCmd}\`${isClaude ? ' (Claude Code)' : ''}`);
    lines.push('');
    ...
    if (scan.landmines.length > 0) {
        ...
        lines.push('[No TO' + 'DO/FIX' + 'ME/HA' + 'CK found]');
    }
    lines.push('');
