...
function detectExistingProject(dir) {
    ...
    // Tier 1: Nhận diện qua file Manifest chuẩn (package.json, pyproject.toml, go.mod...)
    ...
    // Tier 2: Nhận diện qua thư mục .omni đã được khởi tạo
    const hasOmni = fs.existsSync(path.join(dir, '.omni', 'manifest.json'));
    if (hasOmni && langs.length === 0) {
        langs.push('Generic (Omni-initialized)');
    }

    // Tier 3: Nhận diện Git repository
    const hasGit = fs.existsSync(path.join(dir, '.git'));

    // Quét các file ở thư mục gốc để phân tích thêm
    let files = 0;
    let dirs = 0;
    const rootFiles = [];
    try {
    ...
    // Tier 4: Nhận diện qua định dạng file nguồn ở thư mục gốc (C#, Python, Go...)
    if (langs.length === 0) {
        const extLangs = new Set();
        for (const file of rootFiles) {
            const ext = path.extname(file).toLowerCase();
            if (ext === '.cs') extLangs.add('C#');
            else if (ext === '.py') extLangs.add('Python');
            else if (ext === '.go') extLangs.add('Go');
            else if (ext === '.rs') extLangs.add('Rust');
            else if (ext === '.java') extLangs.add('Java');
            else if (ext === '.cpp' || ext === '.c' || ext === '.h') extLangs.add('C/C++');
            else if (ext === '.sh') extLangs.add('Shell');
            else if (ext === '.html' || ext === '.css' || ext === '.js') extLangs.add('Web');
        }
        
        // Kiểm tra thêm file .csproj hoặc .sln của C# / WinForms
        const hasCsharp = rootFiles.some(f => f.endsWith('.csproj') || f.endsWith('.sln'));
        if (hasCsharp) extLangs.add('C#');
    ...
    // Tier 5: Chấp nhận làm dự án Generic nếu có thư mục Git hoặc có chứa file/folder bất kỳ
    if (langs.length === 0 && (hasGit || files > 0 || dirs > 0)) {
        langs.push('Generic');
    }

    // Nếu hoàn toàn trống rỗng và không có chỉ dấu nào -> Không phát hiện project
    if (langs.length === 0) {
        return { detected: false, stats: { files: 0, dirs: 0 }, lang: '' };
    }

    // Tối ưu hoá nhận diện TypeScript từ Node.js
