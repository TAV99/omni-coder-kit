...
1. **Think Before Coding (Socratic Gate & Spec Intake):** Don't assume. Don't hide confusion. Surface tradeoffs.
   - **MANDATORY:** Before writing ANY code for a new feature or complex change, you MUST ask at least 3 clarifying questions (or execute `>om-think`). This is NON-NEGOTIABLE — even if the user says "just do it."
...
2. **Simplicity First (Anti-Overengineering):** Minimum code that solves the problem. Nothing speculative.
...
3. **Surgical Changes (Scope Lock):** Touch only what you must. Clean up only your own mess.
...
4. **Goal-Driven Execution (Verifiable Quality Gates):** Define success criteria. Loop until verified.
   - Transform vague tasks into verifiable goals:
     - "Add validation" → "Write tests for invalid inputs, then make them pass (`>om-check`)"
     - "Fix the bug" → "Write a test that reproduces it, then make it pass (`>om-fix`)"
     - "Refactor X" → "Ensure tests pass before and after (`>om-check`)"
