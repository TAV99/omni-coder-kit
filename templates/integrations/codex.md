...
- **Stable Omni Commands:** Type `>om-think`, `>om-plan`, `>om-cook`, etc. as normal chat text. Do not rely on custom `/om-*` slash commands in Codex.
- **Alias Commands:** You may type `$om-think`, `$om-plan`, `$om-cook`, etc. anywhere in normal text; treat them as `>om-*` commands.
- **Alias Escape:** Ignore `$om-*` tokens inside inline backticks and fenced code blocks.
