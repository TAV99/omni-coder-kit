...
- **Sub-Agent Execution:** Khi `/om:cook` chạy, phân tích dependency graph trong `todo.md` và spawn parallel agents (worktree isolation) cho tasks độc lập. Xem chi tiết: `.omni/workflows/coder-execution.md`
- **Task Tracking:** Dùng TaskCreate/TaskUpdate để track progress khi thực thi tasks, thay vì chỉ dựa vào `todo.md` checkboxes.
