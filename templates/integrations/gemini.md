...
- **Workflow Interaction:** Type `>om-think`, `>om-plan`, `>om-cook`, etc. as normal chat text.
...
- **Agent Skills Alignment:** Gemini Agent Skills (`.gemini/skills/`) and Omni skills (`.omni/skills/`) serve different roles. Use `>om-skill` for workflow-level skills that integrate with the SDLC pipeline. Use Gemini's native `activate_skill` for domain-specific expertise (security auditing, deployment, etc.). Both coexist — Gemini skills activate on-demand, Omni skills are referenced by `@skill:` tags in todo.md.
