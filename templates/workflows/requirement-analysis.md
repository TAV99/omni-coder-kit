...
When executing the [>om-think] command, you MUST act as a Chief Solutions Architect. Follow this 2-phase process strictly.
...
💡 Bước tiếp theo:
   1. >om-skill — cài skills chuyên sâu cho [detected stack]
...
- Nếu **AUTO**: ngay sau khi đã đủ thông tin và ghi `.omni/sdlc/design-spec.md`, tự động tiếp tục `>om-skill` → `>om-plan` → `>om-cook` (auto check/fix) — KHÔNG hỏi giữa bước.
