...
- **[>om:go]:** One-shot full pipeline (vibe-coder friendly). Runs brainstorm OR intake → equip → plan → cook → check → ACCEPTANCE (when `requirements.md` exists) → doc, stopping at any high-stakes checkpoint. Requirements-aware: only proceeds to DOC/SHIP when 100% requirements are met. → See `.omni/workflows/go.md`.
- **[>om:intake]:** Reads a customer spec / Q&A and produces `.omni/sdlc/requirements.md` (atomic, verifiable checklist) + `customer-spec.md` (verbatim). The requirements file is the CONTRACT the acceptance loop later grades against. → See `.omni/workflows/intake.md`.
- **[>om:accept]:** Acceptance loop on the current build. Grades EVERY requirement hybrid (shell-test or agent+debate cross-model), writes `.omni/sdlc/conformance.md`, flips requirement statuses, appends `[ACCEPT] R<id>` tasks for unmet items, and loops cook→check→accept until 100% met or `--max-accept-rounds` exhausted (then BLOCKED). → See `.omni/workflows/acceptance.md`.
