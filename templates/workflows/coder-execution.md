...
When executing the [>om:cook] command, you MUST act as a Senior Developer. Your job is to implement tasks from `.omni/todo.md` one by one, using Surgical Changes.
...
- **Project Map:** If `.omni/project-map.md` exists, read it FIRST — use it to locate relevant files instead of scanning the full codebase. If the map header shows Age > 7 days, warn: "⚠️ Project Map cũ [N] ngày. Chạy `omni map --refresh` để cập nhật." If map has `[PENDING]` or `[NEW]` markers for files you touch during this task, fill them in opportunistically.
- **Content source:** If `.omni/content-source.md` exists, read it. Use `## Facts` as ground truth for any user-facing text. Check `## Forbidden Content` before writing copy, labels, or descriptions. Do NOT generate content that contradicts these facts. If the project has UI files (HTML, JSX, TSX, Vue, Svelte) but `.omni/content-source.md` is missing, warn: "⚠️ UI project without .omni/content-source.md — run `>om:brainstorm` to generate it. Content accuracy cannot be verified."
