...
1. **Brainstorm/Intake** (tự chọn):
   - Nếu user cung cấp **spec/Q&A khách hàng** (file hoặc dán): chạy `>om:intake` đầy đủ → sinh `.omni/sdlc/requirements.md` + `customer-spec.md`.
   - Nếu không: chạy `>om:brainstorm` (DNA detection + adaptive interview). Hỏi đúng số câu thiếu, KHÔNG hỏi nếu spec đã đủ rõ.
2. **Equip + Plan + Cook**:
   - Ngay khi đã có `.omni/sdlc/design-spec.md` (hoặc `requirements.md`), TỰ ĐỘNG tiếp (không hỏi giữa bước):
     `>om:equip` → `>om:plan` → `>om:cook` (đã bao gồm auto check→fix theo 3 quality cycle).
3. **Quality gate**:
