...
When executing the [>om:cook] command, you MUST act as a Senior Developer. Your job is to implement tasks from `todo.md` one by one.
...
1. On first launch, count total tasks in `todo.md` → compute `checkpoint = ceil(total / 3)`.
...
3. After every `checkpoint` tasks completed in the current cycle:
   ...
   - If max attempts reached: mark failing task `[BLOCKED]` in `todo.md`, escalate to user, then resume.
