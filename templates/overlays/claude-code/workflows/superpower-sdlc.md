...
- **[>om:brainstorm]:** Activates the Solutions Architect Agent. Extracts info from user prompt, classifies complexity (small/medium/large), asks only what's missing (adaptive interview), auto-decomposes large projects, then outputs `.omni/design-spec.md` in hybrid format (summary table + tagged requirement list). → See ADAPTIVE ARCHITECT WORKFLOW section.
- **[>om:equip]:** Activates the Skill Manager Agent. Reads the tech stack from `.omni/design-spec.md` and proposes `npx skills add` commands to fetch necessary expert skills from skills.sh. → See AGENT SKILLS MANAGER section.
- **[>om:plan]:** Activates the PM Agent. Reads `.omni/design-spec.md` and breaks it into detailed micro-tasks in `.omni/todo.md`. Each task must be atomic (<20 min) and use `- [ ]` checkbox format. → See PM AGENT WORKFLOW section.
- **[>om:cook]:** Activates the Coder Agent. Analyzes dependency graph in `.omni/todo.md`, groups independent tasks into batches, spawns parallel sub-agents with worktree isolation. Quality gate triggers at each 1/3 checkpoint. → See CODER AGENT WORKFLOW section.
- **[>om:check]:** Activates the QA Tester Agent. Verifies every completed task in `.omni/todo.md` actually works (build, tests, feature verification). Outputs `.omni/test-report.md`. → See QA TESTING WORKFLOW section.
- **[>om:fix]:** Activates the Debugger Agent. Reads `.omni/test-report.md` or user-reported errors. Reproduces → Root cause analysis → Surgical fix → Verify. Never shotgun-fix. → See DEBUGGER AGENT WORKFLOW section.
