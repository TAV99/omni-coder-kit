    ...
    .action(async () => {
        ...
        const response = await prompts([
            ...
            },
            {
                type: 'multiselect',
                name: 'stacks',
                message: q(3, 3, 'Chọn Tech Stack (BẤM ENTER ĐỂ BỎ QUA - NẾU MUỐN AI TƯ VẤN TRƯỚC):'),
                choices: [
        ...
        const qaTesting = readTemplate(path.join(templatesDir, 'workflows', 'qa-testing.md'));
        const taskPlanning = readTemplate(path.join(templatesDir, 'workflows', 'task-planning.md'));
        const coderExecution = readTemplate(path.join(templatesDir, 'workflows', 'coder-execution.md'));
        const debuggerWorkflow = readTemplate(path.join(templatesDir, 'workflows', 'debugger-workflow.md'));
        const docWriter = readTemplate(path.join(templatesDir, 'workflows', 'documentation-writer.md'));
        const validationScripts = readTemplate(path.join(templatesDir, 'workflows', 'validation-scripts.md'));
        ...
        if (response.ide === 'dual') {
            ...
            if (response.stacks && response.stacks.length > 0) {
                agentsRules += `## TECH STACK SPECIFIC RULES\n\n`;
                for (const stack of response.stacks) {
                    const content = readTemplate(path.join(templatesDir, 'stacks', `${stack}.md`));
                    const { start, end } = getMarkers(stack);
                    agentsRules += `${start}\n${content}\n${end}\n\n`;
